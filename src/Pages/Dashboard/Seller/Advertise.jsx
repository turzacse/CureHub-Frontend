...
const Advertise = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('https://cure-hub-backend-gules.vercel.app/ads', formData);
    ...
    useEffect( () => {
        fetch('https://cure-hub-backend-gules.vercel.app/ads')
        .then(res => res.json())
        .then(data => setAds(data.filter((item) => item?.seller === user.email)))
    } ,[])
    ...
    return (
        <div>
           ...
           <h2 className='text-xl font-bold text-center mb-5'>Your requesting Advertisements:</h2>
           </div >
           <div className='grid md:grid-cols-3 grid-cols-1 gap-2'>
           {
            ads?.map((item) => <div className='bg-[#1A6868] rounded-lg p-2' key={item._id}>
                ...
                <p className='text-white mt-4 mb-8'>{item.description}</p>
                <p>{item.status}</p>
            </div>)
           }
           </div>
